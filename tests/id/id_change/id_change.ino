...
void setup() {
  ...
  // IMPORTANT: Must match AX-18A's current baud rate
  dxl.begin(1000000);

  dxl.setPortProtocolVersion(DXL_PROTOCOL_VERSION);

  if(dxl.ping(present_id)) {

    DEBUG_SERIAL.println("AX-18A found!");

    dxl.torqueOff(present_id);

    if(dxl.setID(present_id, new_id)) {

      DEBUG_SERIAL.print("ID successfully changed from ");
    ...
    DEBUG_SERIAL.println("AX-18A not found!");
  }
}
